  ...
  const cargarEvaluacion = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && response.data.status === 'success') {
        ...
        // Intentar obtener puntuación y valoración con diferentes nombres de campos
        let puntuacion = null;
        let valoracion = null;
        
        if (esTrimestral) {
          // Para trimestrales, buscar en múltiples campos posibles
          puntuacion = evaluacionData.puntuacionTotal || evaluacionData.puntuacion_total || evaluacionData.calificacion;
          valoracion = evaluacionData.valoracionTrimestral || evaluacionData.valoracion_trimestral || evaluacionData.valoracion;
        } else {
          // Para anuales, buscar en múltiples campos posibles
          puntuacion = evaluacionData.puntuacionTotal || evaluacionData.puntuacion_total || evaluacionData.calificacion;
          valoracion = evaluacionData.valoracionAnual || evaluacionData.valoracion_anual || evaluacionData.valoracion;
        }
        
        // Si tenemos datos, establecerlos
        if (puntuacion !== null && puntuacion !== undefined) {
          setCalificacionTotal(puntuacion);
        }
        
        if (valoracion && valoracion !== '') {
          setValoracionFinal(valoracion);
        }
        ...
        if (evaluacionData.respuestas && evaluacionData.respuestas.length > 0) {
          ...
          const tienePuntuacion = evaluacionData.puntuacionTotal || evaluacionData.puntuacion_total || evaluacionData.calificacion;
          const tieneValoracion = esTrimestral 
            ? (evaluacionData.valoracionTrimestral || evaluacionData.valoracion_trimestral || evaluacionData.valoracion)
            : (evaluacionData.valoracionAnual || evaluacionData.valoracion_anual || evaluacionData.valoracion);
          
          if (!tienePuntuacion || !tieneValoracion) {
